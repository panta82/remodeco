...
use crate::model::{Journal, ManifestFile, SessionRecord};
use crate::util::now;
...
fn session_lock_path(id: &str) -> Result<PathBuf> {
    Ok(session_dir(id)?.join("session.lock"))
}
pub fn require_session_lock(id: &str) -> Result<HeldLock> {
    require_lock(
        &session_lock_path(id)?,
        &format!("session already locked: {id}"),
    )
}
...
pub fn delete_session_dir(id: &str) -> Result<bool> {
